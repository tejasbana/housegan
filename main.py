...
for epoch in range(opt.n_epochs):
    total_gloss = []
    total_dloss = []
    for i, batch in tqdm(enumerate(fp_loader)):
        ...
        if i % opt.n_critic == 0:
            ...
            total_gloss.append(g_loss.detach())
            total_dloss.append(d_loss.detach())
    ...
    print("[Epoch %d/%d] [Batch %d/%d] [D loss: %f] [G loss: %f]"
        % (epoch, opt.n_epochs, i, len(fp_loader), torch.stack(total_dloss).mean(), torch.stack(total_gloss).mean()))
